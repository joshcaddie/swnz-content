...
import type { FieldType, Structure, StructurePage } from '../lib/database.types'
...
import { iconFor } from '../fields/registry'
...
export function WizardPage() {
  ...
  // Apply ?template=id once templates load.
  ...
  // ---- structure editing (in-memory until finalize) ----
  const mutate = (fn: (s: Structure) => Structure) => setStructure((s) => fn(structuredCloneSafe(s)))
  const addPage = () => mutate((s) => { s.pages.push({ name: 'Untitled page', sections: [] }); return s })
  const renamePage = (pi: number, v: string) => mutate((s) => { s.pages[pi].name = v; return s })
  const deletePage = (pi: number) => { mutate((s) => { s.pages.splice(pi, 1); return s }); setActivePage(0) }
  ...
  const deleteSection = (si: number) => mutate((s) => { s.pages[activePage].sections.splice(si, 1); return s })
  const addField = (type: FieldType, label: string) => mutate((s) => {
    const page = s.pages[activePage]
    if (page.sections.length === 0) page.sections.push({ name: 'Untitled section', fields: [] })
    page.sections[page.sections.length - 1].fields.push({ type, label })
    return s
  })
  const deleteField = (si: number, fi: number) => mutate((s) => { s.pages[activePage].sections[si].fields.splice(fi, 1); return s })
  const renameField = (si: number, fi: number, v: string) => mutate((s) => { s.pages[activePage].sections[si].fields[fi].label = v; return s })
  ...
  const finalize = async () => {
    ...
    try {
      ...
      // best-effort: invite the client by email (needs SMTP2GO configured)
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, background: C.panel }}>
      ...
      {step === 1 && (
        <TemplatesStep
          ...
          onPick={(s, nm) => { setStructure(s); setName(nm); setStep(2) }}
        />
      )}
      ...
      {step === 3 && (
        <BuilderStep
          structure={structure} activePage={activePage} setActivePage={setActivePage} ap={ap} name={name}
          ...
          deleteField={deleteField} renameField={renameField}
...
function structuredCloneSafe(s: Structure): Structure {
  return JSON.parse(JSON.stringify(s)) as Structure
}
  ...
  return (
    <div className="swnz-scroll" style={{ flex: 1, overflowY: 'auto', padding: 38, display: 'flex', justifyContent: 'center' }}>
      <div style={{ width: 760, maxWidth: '100%', background: '#fff', border: '1px solid #e9e8ee', borderRadius: 18, padding: '38px 42px', boxShadow: '0 4px 16px rgba(40,30,60,.06)', height: 'fit-content' }}>
        ...
        <Toggle on={p.reminders} onToggle={() => p.setReminders(!p.reminders)} text="Send automated reminders to the client" />
        <Toggle on={p.verifyEmail} onToggle={() => p.setVerifyEmail(!p.verifyEmail)} text="Require the client to verify their email with a code" />
      </div>
    </div>
  )
}

function Toggle({ on, onToggle, text }: { on: boolean; onToggle: () => void; text: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 24, paddingTop: 22, borderTop: '1px solid #f0eff3' }}>
      <div onClick={onToggle} style={{ width: 50, height: 28, borderRadius: 16, background: on ? C.navy2 : '#d8d6df', position: 'relative', cursor: 'pointer', flex: 'none' }}>
...
function BuilderStep(p: {
  ...
  deleteField: (si: number, fi: number) => void; renameField: (si: number, fi: number, v: string) => void
  openFieldModal: () => void
}) {
  return (
    <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
      {/* pages */}
      <div style={{ width: 330, flex: 'none', background: '#fff', borderRight: `1px solid ${C.line}`, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          {p.structure.pages.map((pg, pi) => (
            <div key={pi}>
              <div onClick={() => p.setActivePage(pi)} className="swnz-hover swnz-hover-grey" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 12px', borderRadius: 8, cursor: 'pointer' }}>
                <span style={{ fontWeight: 800, fontSize: 18, flex: 1, color: pi === p.activePage ? C.cyan : C.ink }}>{pi + 1}. {pg.name}</span>
      ...
      <div className="swnz-scroll" style={{ flex: 1, overflowY: 'auto', padding: '28px 36px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginTop: 30 }}>
          <input value={p.ap.name} onChange={(e) => p.renamePage(p.activePage, e.target.value)} style={{ fontWeight: 800, fontSize: 24, color: C.inkDark, border: 'none', outline: 'none', background: 'transparent', flex: 1 }} />
        </div>
        ...
        {p.ap.sections.map((sec, si) => (
          <div key={si} style={{ background: '#fff', border: '1.5px solid #bfe0f2', borderRadius: 16, padding: 24, marginTop: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span onClick={() => p.deleteSection(si)} style={{ color: '#c9491f', fontWeight: 800, fontSize: 16, cursor: 'pointer' }}>Delete section</span>
            </div>
            {sec.fields.map((f, fi) => (
              <div key={fi}>
                {f.tag && <div style={{ display: 'inline-block', background: '#e6f3fb', color: C.navy2, fontWeight: 600, fontSize: 14, padding: '5px 12px', borderRadius: 7, margin: '16px 0 0' }}>{f.tag}</div>}
                <div style={{ background: '#fff', border: '1px solid #ededf1', borderRadius: 10, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, marginTop: 12, boxShadow: '0 1px 3px rgba(0,0,0,.04)' }}>
                  <span style={{ color: C.navy2, fontSize: 18 }}>{iconFor(f.type)}</span>
                  <input value={f.label} onChange={(e) => p.renameField(si, fi, e.target.value)} style={{ fontWeight: 600, fontSize: 18, color: C.ink, flex: 1, border: 'none', outline: 'none', background: 'transparent' }} />
                  <span onClick={() => p.deleteField(si, fi)} style={{ color: '#cfcdd6', fontWeight: 800, fontSize: 17, cursor: 'pointer' }}>✕</span>
                </div>
              </div>
            ))}
            <div style={{ display: 'flex', gap: 14, marginTop: 22 }}>
              <div onClick={p.openFieldModal} style={{ border: `1.5px solid ${C.navy2}`, color: C.navy2, fontWeight: 800, fontSize: 13, letterSpacing: '0.5px', padding: '12px 20px', borderRadius: 24, cursor: 'pointer' }}>ADD A FIELD</div>
            ...
            <div key={`${pi}-${si}`} style={{ background: '#fff', border: '1px solid #e9e8ee', borderRadius: 16, padding: '32px 36px', marginBottom: 20, boxShadow: '0 2px 10px rgba(40,30,60,.05)' }}>
              ...
              {sec.fields.map((f, fi) => (
                <div key={fi} style={{ marginBottom: 24 }}>
                  <div style={{ fontWeight: 700, fontSize: 18, color: C.ink, marginBottom: 10 }}>{f.label}</div>
                  <div style={{ minHeight: 80, background: C.panel, border: '1px solid #e3e2e8', borderRadius: 10, padding: '14px 16px', color: '#a8a4b0', fontSize: 16 }}>Client response…</div>
                </div>
              ))}
            </div>
          )),
