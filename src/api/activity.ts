...
/** Event feed derived from existing tables — no extra writes needed. */
export function useActivity(limit = 60) {
  return useQuery({
    queryKey: ['activity', limit],
    queryFn: async (): Promise<ActivityEvent[]> => {
      const [{ data: requests }, { data: answers }, { data: comments }] = await Promise.all([
        supabase.from('requests').select('id, name, created_at').order('created_at', { ascending: false }).limit(limit),
      ...
      for (const a of answers ?? []) {
        const rn = nameById.get(a.request_id) ?? 'A request'
      ...
      for (const c of comments ?? []) {
        events.push({ id: `com-${c.id}`, kind: 'comment', requestId: c.request_id, requestName: nameById.get(c.request_id) ?? 'A request', detail: `${c.author_name ?? 'Someone'}: ${c.body.slice(0, 80)}`, at: c.created_at })
      }
...
/** Count of answers awaiting review — powers the bell badge. */
export function useAwaitingCount() {
  return useQuery({
    queryKey: ['awaiting-count'],
    refetchInterval: 60_000,
    queryFn: async (): Promise<number> => {
      const { count } = await supabase.from('answers').select('id', { count: 'exact', head: true }).eq('status', 'submitted')
      return count ?? 0
    },
  })
}
