-- Page hierarchy: sub-pages indent under the nearest top-level page above them,
-- and a page can be a navigation label only (no content collected).

alter table request_pages
  add column if not exists indent int not null default 0,
  add column if not exists nav_only boolean not null default false;

-- Recreate the structure functions so templates round-trip the new fields.
